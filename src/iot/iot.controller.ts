      ...
      properties: {
        ...
        businessHoursAnalysis: {
          ...
          properties: {
            organizationHours: {
              ...
              properties: {
                openTime: { type: 'string', example: '08:00', description: 'Business opening time (HH:mm)' },
                closeTime: { type: 'string', example: '17:00', description: 'Business closing time (HH:mm)' },
                timezone: { type: 'string', example: 'Africa/Johannesburg', description: 'Organization timezone' },
                isHoliday: { type: 'boolean', example: false, description: 'Whether today is marked as holiday' }
              }
            },
            eventAnalysis: {
              type: 'object',
              ...
              properties: {
                ...
                eventTime: { type: 'string', example: '08:00', description: 'Event time in organization timezone (HH:mm)' },
                isWithinBusinessHours: { type: 'boolean', example: true, description: 'Whether event occurred during business hours' },
                attendanceStatus: { 
                  type: 'string', 
                  example: 'ON_TIME',
                ...
                minutesFromSchedule: {
                  type: 'number',
                  example: 0,
                  description: 'Minutes difference from scheduled time (positive = late, negative = early)'
                },
                workingDay: {
                  type: 'boolean',
                  example: true,
        ...
        analytics: {
          ...
          properties: {
            deviceAnalytics: {
              type: 'object',
              properties: {
                totalEvents: { type: 'number', example: 156, description: 'Total events recorded by this device' },
                ...
                punctualityRate: { type: 'number', example: 89.7, description: 'Punctuality percentage' }
              }
            },
            attendanceMetrics: {
              type: 'object',
              properties: {
                currentMonth: {
                  type: 'object',
                  properties: {
                    totalWorkingDays: { type: 'number', example: 22, description: 'Working days in current month' },
                    daysPresent: { type: 'number', example: 20, description: 'Days with recorded events' },
                    attendanceRate: { type: 'number', example: 90.9, description: 'Attendance percentage' },
                    avgArrivalTime: { type: 'string', example: '08:05', description: 'Average arrival time' },
                    avgDepartureTime: { type: 'string', example: '17:15', description: 'Average departure time' }
                  }
                }
              }
            }
        ...
        device: {
          ...
          properties: {
            ...
            deviceType: { type: 'string', example: 'DOOR_SENSOR', description: 'Type of IoT device' },
            location: { type: 'string', example: 'Main Entrance, Building A', description: 'Physical device location' },
            status: { type: 'string', example: 'ONLINE', description: 'Current device operational status' },
            lastSeen: { type: 'string', format: 'date-time', example: '2023-01-05T08:00:00.000Z', description: 'Last communication timestamp' },
            organization: {
              type: 'object',
              properties: {
        ...
        processing: {
          ...
          properties: {
            processingTime: { type: 'number', example: 145, description: 'Processing time in milliseconds' },
            source: { type: 'string', example: 'url_parameters', description: 'Data source method' },
            validationChecks: {
              type: 'object',
              properties: {
                deviceValidation: { type: 'boolean', example: true, description: 'Device registration check passed' },
                permissionValidation: { type: 'boolean', example: true, description: 'User permission check passed' },
                timestampValidation: { type: 'boolean', example: true, description: 'Timestamp format check passed' },
                businessHoursValidation: { type: 'boolean', example: true, description: 'Business hours check completed' }
              }
            },
            cacheUpdates: {
              type: 'array',
  ...
  @ApiNotFoundResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        notFoundType: {
          type: 'string',
          enum: ['DEVICE_NOT_FOUND', 'ORGANIZATION_NOT_FOUND', 'BUSINESS_HOURS_NOT_CONFIGURED', 'BRANCH_NOT_FOUND'],
  ...
  @ApiForbiddenResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        accessViolationType: {
          type: 'string',
          enum: ['ORGANIZATION_ACCESS_DENIED', 'BRANCH_ACCESS_DENIED', 'DEVICE_ACCESS_DENIED', 'INSUFFICIENT_ROLE'],
      ...
      properties: {
        ...
        tokenInfo: {
          ...
          properties: {
            ...
            location: { type: 'string', example: 'Authorization header', description: 'Where token should be provided' }
          },
        ...
        resolution: {
          type: 'object',
          properties: {
            loginEndpoint: { type: 'string', example: 'POST /auth/login', description: 'Endpoint to obtain token' },
            tokenFormat: { type: 'string', example: 'Authorization: Bearer <jwt_token>', description: 'Required header format' },
            documentation: { type: 'string', example: '/docs/authentication', description: 'Authentication documentation' }
          },
  ...
  @ApiConflictResponse({
    ...
    schema: {
      ...
      properties: {
        ...
        conflictType: {
          type: 'string',
          enum: ['DUPLICATE_EVENT', 'BUSINESS_HOURS_VIOLATION', 'DEVICE_STATE_CONFLICT', 'ATTENDANCE_POLICY_VIOLATION'],
        ...
        businessImpact: {
          type: 'object',
          properties: {
            attendanceAffected: { type: 'boolean', example: true, description: 'Whether attendance calculations are affected' },
            analyticsImpacted: { type: 'boolean', example: false, description: 'Whether device analytics are impacted' },
            complianceIssue: { type: 'boolean', example: false, description: 'Whether this creates compliance problems' }
          },
        ...
        resolutionOptions: {
          ...
          items: {
            type: 'object',
            properties: {
              action: { type: 'string', example: 'UPDATE_EXISTING', description: 'Resolution action type' },
              description: { type: 'string', example: 'Update the existing record with new metadata', description: 'Description of the action' },
              endpoint: { type: 'string', example: 'PATCH /iot/records/123', description: 'API endpoint for resolution' }
            }
          },
          example: [
            {
    ...
    schema: {
      ...
      properties: {
        ...
        systemContext: {
          type: 'object',
          properties: {
            component: { type: 'string', example: 'IoT Event Processor', description: 'System component where error occurred' },
            operation: { type: 'string', example: 'RECORD_TIME_EVENT', description: 'Operation being performed' },
            errorId: { type: 'string', example: 'ERR_20230105_080001_ABC123', description: 'Unique error identifier for tracking' }
          },
        ...
        supportInfo: {
          ...
          properties: {
            ...
            includeErrorId: { type: 'boolean', example: true, description: 'Whether to include error ID when contacting support' },
            urgencyLevel: { type: 'string', example: 'MEDIUM', enum: ['LOW', 'MEDIUM', 'HIGH', 'CRITICAL'], description: 'Support urgency level' }
          },
